...
Ideaspark ESP8266 0.96" OLED + SHT30 Temperature and Humidity Outdoor Sensor
 - connect to Wifi
...
####################################################################################
# Get SHT30 sensor readings
####################################################################################

async def measureSHT30():
    ''' do SHT30 measurements after config.DHT_INTERVAL seconds and save results in global variables '''

    # SHT30 object
    sensor = SHT30(scl_pin=config.SCL_GPIO, sda_pin=config.SDA_GPIO, i2c_address=68)
    ...
    while True:
        ...
        myGlobals.temp = temperature
        myGlobals.hum = humidity
        
        # debug output
        if config.DEBUG:
            print('Temp: {:0.2f}°C - Hum: {:0.2f}%'.format(myGlobals.temp, myGlobals.hum))
...
async def refreshOLED():
    ...
    # I2C object
    i2c = I2C(scl=Pin(config.SCL_GPIO), sda=Pin(config.SDA_GPIO), freq=100000)
    ...
    while True:
        ...
        font_writer30.printstring('{:0.0f}C {:0.0f}%' .format(myGlobals.temp, myGlobals.hum))
